...
df1 |> ggplot(aes(sample_position, `marker value`, col=time)) +
  ...
  labs(title = "4 dot Marker data overview")
```
...
df1 |> ggplot(aes(sample_position, `marker value`, col = time)) +
  geom_line() +
  labs(title = "Distribution of 4 dot Marker based on time point")
```
...
for (time_val in unique(newdf$time)) {
  for (type_val in unique(newdf$type)) {
    ...
    model <- lm(`marker value` ~ poly(sample_position, degree = 5), data = subset_data)
